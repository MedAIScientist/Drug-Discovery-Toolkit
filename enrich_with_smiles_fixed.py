...
class DrugDataEnricher:
        ...
        try:
            ...
            if smiles_file.exists():
                with open(smiles_file, "r") as f:
                    ...
                    smiles_dict = json.loads(content)

                logger.info(f"Loaded {len(smiles_dict)} SMILES from DAVIS")
                ...
                if target_file.exists():
                    with open(target_file, "r") as f:
                        ...
                        content = f.read().strip()
                        try:
                            targets_dict = json.loads(content)
                            targets = list(targets_dict.values())
                        except json.JSONDecodeError:
                            # If not JSON, read as lines
                            f.seek(0)
                ...
                for i, (drug_id, smiles) in enumerate(
                    tqdm(smiles_dict.items(), desc="Processing DAVIS drugs")
                ):
                    drug_key = f"davis_{drug_id}"
                ...
                if affinity_matrix is not None and targets:
                    self._add_davis_interactions(
                        list(smiles_dict.values()), targets, affinity_matrix
                    )
    ...
    def _add_davis_interactions(
        ...
        try:
            ...
            davis_drug_ids = list(
                json.loads(open("./data/DAVIS/SMILES.txt").read()).keys()
            )
            ...
            for i in range(min(len(drugs), affinity_matrix.shape[0])):
                for j in range(min(len(targets), affinity_matrix.shape[1])):
                    ...
                    if affinity >= threshold:
                        drug_key = (
                            f"davis_{davis_drug_ids[i]}"
                            if i < len(davis_drug_ids)
                            else f"davis_drug_{i}"
                        )
                        edge_doc = {
                            "_from": f"drugs/{drug_key}",
    ...
    def load_drugbank_vocabulary(self):
        ...
        try:
            ...
            id_columns = [
                col for col in df.columns if "DRUGBANK" in col.upper() or col == "ID"
            ]

            if smiles_columns and id_columns:
                ...
                for idx, row in df.iterrows():
                    ...
                    if db_id and smiles and pd.notna(smiles) and pd.notna(db_id):
