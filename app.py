...
class ConversationMemory:
    ...
    def __init__(self, session_id: str, max_tokens: int = 4000):
        ...
        self.chat_history: ChatHistory = ChatHistory(session_id)
    ...
    def on_agent_action(self, action, **kwargs):
        ...
        if hasattr(action, "log") and action.log:
            ...
            if action.tool == "FindDrug":
                ...
                thought_content = (
                    f"Let me get molecular information for: {action.tool_input}"
                )
            elif action.tool == "PlotSmiles3D":
                thought_content = (
                    f"I'll generate a 3D structure from the SMILES: {action.tool_input}"
                )
            else:
                thought_content = (
                    f"I'll use the {action.tool} tool to help answer this question"
                )
...
def agent_executor(user_query: str, conversation_memory: ConversationMemory):
    ...
    try:
        ...
        reasoning_summary = " | ".join(
            [step.get("content", "") for step in callback_handler.reasoning_steps]
        )
...
if user_input := st.chat_input("Type your drug-related query..."):
    ...
    st.sidebar.empty()
    st.sidebar.markdown(
        f"<div style='text-align: center;'><img src='data:image/png;base64,{img_base64}' width='175'></div>",
        unsafe_allow_html=True,
    )
    st.sidebar.markdown(
        "<h1 style='text-align: center; color: #F3BB4F; font-size: 2rem;'>Research Agent</h1>",
        unsafe_allow_html=True,
    )
    st.sidebar.divider()
