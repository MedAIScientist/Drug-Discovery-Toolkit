...
drug_collection = db.collection("drug") if db else None
link_collection = db.collection("drug-protein") if db else None

tokenizer = AutoTokenizer.from_pretrained("seyonec/ChemBERTa-zinc-base-v1")
model = AutoModel.from_pretrained("seyonec/ChemBERTa-zinc-base-v1")

# Initialize TamGen worker if checkpoint exists
worker = None
try:
    if os.path.exists("checkpoints/crossdock_pdb_A10/checkpoint_best.pt"):
        worker = TamGenCustom(
            data="./TamGen_Demo_Data",
            ckpt="checkpoints/crossdock_pdb_A10/checkpoint_best.pt",
            use_conditional=True,
        )
        print("TamGen worker initialized successfully")
    else:
        print(
            "Warning: TamGen checkpoint not found. Molecular generation features will be disabled."
        )
except Exception as e:
    print(f"Warning: Failed to initialize TamGen worker: {e}")
    print("Molecular generation features will be disabled.")
    worker = None
...
def _SanitizeInput(d: Any, list_limit: int) -> Any:
    ...
    if isinstance(d, dict):
        ...
        if len(d) == 0:
            ...
            return [
                _SanitizeInput(item, list_limit)
                for item in d
                if _SanitizeInput(item, list_limit) is not None
            ]
        else:
...
def _display_sidebar_output(
    title: str, content: Any, content_type: str = "json"
) -> None:
...
def FindDrug(drug_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        if not db:
            _display_sidebar_output("Database Error", "ArangoDB not available", "error")
            return None
        ...
        if results:
            ...
            _display_sidebar_output(
                "Drug Search", f"No results found for: {drug_name}", "error"
            )
            return None

    except Exception as e:
        _display_sidebar_output(
            "Database Error", f"Error searching for drug: {str(e)}", "error"
        )
        return None
...
def FindProteinsFromDrug(drug_name: str) -> List[str]:
    ...
    try:
        ...
        if not proteins:
            _display_sidebar_output(
                "Protein Search", f"No proteins found for drug: {drug_name}", "error"
            )
            return []
        ...
        _display_sidebar_output(
            "Network Analysis Error", f"Error finding proteins: {str(e)}", "error"
        )
        return []
                    ...
                    if "AQL Query" in text:
                        ...
                        try:
                            ...
                            if "syntax error" in text.lower():
                                _display_sidebar_output(
                                    "Syntax Error", text.strip(), "error"
                                )
                            else:
                                _display_sidebar_output(
                                    "Generated AQL", text.strip(), "code"
                                )
...
def PlotSmiles2D(smiles: str) -> bool:
    ...
    try:
        ...
        if mol:
            ...
            _display_sidebar_output(
                "2D Structure",
                f"Successfully rendered structure for: {smiles}",
                "success",
            )
            return True
        else:
            _display_sidebar_output(
                "Invalid SMILES", f"Cannot parse SMILES: {smiles}", "error"
            )
            return False
    except Exception as e:
        _display_sidebar_output(
            "Rendering Error", f"Failed to render 2D structure: {str(e)}", "error"
        )
        return False
...
def PlotSmiles3D(smiles: str) -> bool:
    ...
    try:
        ...
        if status == -1:
            _display_sidebar_output(
                "3D Generation Error", "Failed to generate 3D coordinates", "error"
            )
            return False
        ...
        try:
            ...
            _display_sidebar_output(
                ...
                "UFF optimization failed, using unoptimized structure",
                "error",
            )
        ...
        if not conformer.Is3D():
            _display_sidebar_output(
                "3D Validation Error", "Generated structure is not 3D", "error"
            )
            return False
        ...
        _display_sidebar_output(
            "2D Reference Structure", "Generated from same SMILES", "success"
        )
        with st.sidebar:
        ...
        _display_sidebar_output(
            ...
            f"Failed to generate 3D structure: {str(e)}",
            "error",
        )
        return False
...
def PredictBindingAffinity(
    input_data: Union[str, Dict], y: List[float] = [7.635]
) -> float:
    """
    ...
    try:
        ...
        print(
            f"Predicting binding affinity for drug: {x_drug[:50]}... target: {x_target[:50]}..."
        )
    ...
    try:
        ...
        for model in structure:
            for chain in model:
                ...
                seq = "".join(
                    residue.resname for residue in chain if residue.id[0] == " "
                )
        ...
        if sequences:
            _display_sidebar_output(
                "Sequence Extraction",
                {
                    ...
                    "longest_chain": max(len(seq) for seq in sequences.values())
                    if sequences
                    else 0,
                },
            )
            ...
            _display_sidebar_output(
                "No Sequences", f"No amino acid sequences found in {pdb_id}", "error"
            )
...
def GetChemBERTaEmbeddings(smiles: str) -> Optional[List[float]]:
    ...
    try:
        if not isinstance(smiles, str) or not smiles.strip():
            _display_sidebar_output(
                "Invalid Input", "SMILES string cannot be empty", "error"
            )
            return None
...
def PreparePDBData(pdb_id: str) -> bool:
    ...
    try:
        ...
        if not worker:
            _display_sidebar_output(
                "TamGen Not Available",
                "TamGen worker not initialized. Cannot prepare PDB data.",
                "error",
            )
            return False
        ...
        for ff in FF:
            if f"gen_{out_split}" in ff:
                ...
                _display_sidebar_output(
                    "PDB Data Status", f"Data for {pdb_id} already prepared", "success"
                )
                return True
        ...
        _display_sidebar_output(
            "PDB Preparation", f"Processing PDB data for {pdb_id}...", "success"
        )

        # Run preparation script
        script_path = os.path.abspath("TamGen/scripts/build_data/prepare_pdb_ids.py")
        exit_code = os.system(
            f"python {script_path} tmp_pdb.csv gen_{out_split} -o {DemoDataFolder} -t {thr}"
        )
        ...
        if exit_code == 0:
            _display_sidebar_output(
                "PDB Preparation Complete",
                f"Successfully prepared data for {pdb_id}",
                "success",
            )
            return True
        else:
            _display_sidebar_output(
                "PDB Preparation Failed",
                f"Failed to prepare data for {pdb_id}",
                "error",
            )
            return False
...
def GenerateCompounds(pdb_id: str) -> Dict[str, Union[str, List[str]]]:
    ...
    try:
        ...
        if not worker:
            error_msg = "TamGen worker not available. Cannot generate compounds."
            _display_sidebar_output("TamGen Not Available", error_msg, "error")
        ...
        _display_sidebar_output(
            "Generation Status",
            f"Generating {num_samples} compounds for {pdb_id}...",
            "success",
        )

        # Generate molecules
        generated_mols, reference_mol = worker.sample(
            m_sample=num_samples, maxseed=max_seed
        )
                ...
                for mol in generated_mols:
                    ...
                    if mol:
                        ...
                        similarity = DataStructs.FingerprintSimilarity(
                            fp_ref, fp, metric=DataStructs.TanimotoSimilarity
                        )
                        gens.append((mol, similarity))

                sorted_mols = [
                    mol for mol, _ in sorted(gens, key=lambda e: e[1], reverse=True)
                ]
            else:
        ...
        if valid_mols:
            img = Draw.MolsToGridImage(
                valid_mols, molsPerRow=3, legends=legends, subImgSize=(200, 200)
            )
            st.write(img)
...
def FindSimilarDrugs(smiles: str, top_k: int = 6) -> List[Dict[str, Union[str, float]]]:
    ...
    try:
        ...
        cursor = db.aql.execute(
            aql_query, bind_vars={"query_vector": embedding, "top_k": top_k}
        )
        results = list(cursor)
        ...
        if results:
            ...
            _display_sidebar_output(
                "No Matches", f"No similar drugs found for: {smiles}", "error"
            )
...
find_drug = Tool(name="FindDrug", func=FindDrug, description=FindDrug.__doc__)
...
text_to_aql = Tool(name="TextToAQL", func=TextToAQL, description=TextToAQL.__doc__)

plot_smiles_2d = Tool(
    name="PlotSmiles2D", func=PlotSmiles2D, description=PlotSmiles2D.__doc__
)

plot_smiles_3d = Tool(
    name="PlotSmiles3D", func=PlotSmiles3D, description=PlotSmiles3D.__doc__
)
...
prepare_pdb_data = Tool(
    name="PreparePDBData", func=PreparePDBData, description=PreparePDBData.__doc__
)
...
find_similar_drugs = Tool(
    name="FindSimilarDrugs", func=FindSimilarDrugs, description=FindSimilarDrugs.__doc__
)
